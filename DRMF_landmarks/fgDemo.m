...
%------------------------------------------------%
% % % Load Test Images
dir='/media/datadisk/database/FG/fera2011_crop/';
image_path = strcat(dir,'fera2011-emotion-test/');
saveLandmark_path=strcat(dir,'cropTestResult/landmarks/');
imglistPath=strcat(image_path,'fera2011_emotion_test_jpg_list.txt');
...
for t=start_num:step:min(end_num,imgnum)
    ...
    if(intimgnum==1||t==start_num||isempty(bdbox)==1)
        bbox_method=0;
    else
        bbox_method=0;
    end
...
%         end
        ...
        data(i).bbox = []; % Face Detection Bounding Box [0;0;col;row]
    ...
    for j=t:min(t+step-1,end_num)
        ...
        landmarkfile=strrep(img_list{j},'.jpg','.txt');
